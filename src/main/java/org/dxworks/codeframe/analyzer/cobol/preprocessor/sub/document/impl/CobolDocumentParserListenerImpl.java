...
import org.dxworks.codeframe.analyzer.cobol.generated.Cobol85PreprocessorParser.ReplaceClauseContext;
import org.dxworks.codeframe.analyzer.cobol.generated.Cobol85PreprocessorParser.ReplacingPhraseContext;
...
public class CobolDocumentParserListenerImpl extends Cobol85PreprocessorBaseListener {
	...
	private final List<String> copyStatements = new ArrayList<>();
	...
	public List<String> copyStatements() {
		return Collections.unmodifiableList(copyStatements);
	}
	...
	public void exitCopyStatement(final Cobol85PreprocessorParser.CopyStatementContext ctx) {
		final String copySourceName = extractCopySourceName(ctx.copySource());
		if (copySourceName != null && !copySourceName.isEmpty()) {
			copyStatements.add(copySourceName);
		}
	...
	private String extractCopySourceName(final Cobol85PreprocessorParser.CopySourceContext copySource) {
		if (copySource == null) {
			return null;
		}

		if (copySource.literal() != null) {
			return normalizeName(copySource.literal().getText());
		}

		if (copySource.cobolWord() != null) {
			return normalizeName(copySource.cobolWord().getText());
		}

		if (copySource.filename() != null) {
			return normalizeName(copySource.filename().getText());
		}
